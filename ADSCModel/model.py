...
class Model(object):
    ...
    def random_choice(self, size):
        
        idx_l = [ i for i in range(len(self.node_embedding)) ]
        rc_idx_l = np.random.choice(np.asarray(idx_l), size)

        self.random_node_embedding = np.asarray([ self.node_embedding[i] for i in rc_idx_l ])
