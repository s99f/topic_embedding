...
cdef unsigned long long fast1_o1 (
    ...
    cdef REAL_t f, g, label
    cdef np.uint32_t target_index
    cdef int d, pos = int(10 * weight), neg = 10 - pos
    #negative_f = pos * negative
...
def train_o1(py_node_embedding, py_edge, py_weight, py_lr, py_negative, py_table,
             py_size=None, py_work=None):

    cdef REAL_t *node_embedding = <REAL_t *>(np.PyArray_DATA(py_node_embedding))
    cdef REAL_t *work = <REAL_t *>(np.PyArray_DATA(py_work))

    cdef REAL_t _weight = py_weight
    ...
    with nogil:
        next_random = fast_o1(negative, table, table_len, node_embedding, node_embedding,
                              size, indexes[1], indexes[0], _weight, _lr,  work, next_random)
        next_random = fast_o1(negative, table, table_len, node_embedding, node_embedding,
                              size, indexes[0], indexes[1], _weight, _lr,  work, next_random)
    return result
