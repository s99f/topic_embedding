    ...
    while len(path) < path_length:
        ...
        if len(G.neighbors(cur)) > 0:
            if rand.random() >= alpha:
                nbr_t = [(nbr, G[cur][nbr]['weight']) for nbr in G.neighbors(cur)]
                nbr_l = [item[0] for item in nbr_t]
                nbr_w = [item[1] for item in nbr_t]
                path.append(rand.choices(nbr_l, weights=nbr_w)[0])
...
def load_txtfile(file_, undirected=True):
    row_l = []
    col_l = []
    data_l = []
    row_num = 0
    ...
    with open(file_, "r") as fp:
        for line in fp:
            row, col, data = line.strip().split("\t")
            row_l.append(int(row))
            col_l.append(int(col))
            data_l.append(float(data))
            row_num = int(row) if row_num < int(row) else row_num
            col_num = int(col) if col_num < int(col) else col_num
    csr_matrix_t = csr_matrix((data_l, (row_l, col_l)), shape=(row_num+1, col_num+1))

    return from_numpy(csr_matrix_t, undirected)
